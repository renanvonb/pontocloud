            ...
            <Accordion type="single" collapsible className="border-t border-border/40">
              {faqs.map((faq, i) => (
                <AccordionItem key={faq.q} value={`item-${i}`} className="border-b border-border/40 border-x-0 rounded-none">
                  <AccordionTrigger className="font-[family-name:var(--font-geist-sans)] font-normal text-[15px] md:text-[18px] tracking-[-0.02em] py-5 px-0 hover:no-underline data-[state=open]:font-medium justify-between text-left">
                    {faq.q}
                  </AccordionTrigger>
                  <AccordionContent className="font-inter text-muted-foreground leading-relaxed text-[13px] md:text-[15px] px-0 tracking-[-0.01em] text-left">
                    {faq.a}
                  </AccordionContent>
                </AccordionItem>
              ))}
